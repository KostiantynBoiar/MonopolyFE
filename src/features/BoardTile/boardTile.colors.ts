...
import type { BoardSpace } from './boardTile.schema';
...
// Surface background per space type; fallback to the board tile CSS var.
export const SPACE_SURFACE_MAP: Partial<Record<SpaceType, string>> = {
  [SpaceType.CHANCE]:   'var(--board-special)',
  [SpaceType.CHEST]:    'var(--board-special)',
...
export function getSpaceHeaderColor(space: BoardSpace): string {
  if (space.color) return PROPERTY_COLOR_MAP[space.color];
  if (space.corner) return CORNER_COLOR_MAP[space.corner];

  switch (space.type) {
    case SpaceType.CHANCE:   return BOARD_TILE_COLORS.propertyYellow;
    case SpaceType.CHEST:    return BOARD_TILE_COLORS.propertyCyan;
    case SpaceType.RAILROAD: return BOARD_TILE_COLORS.railroad;
    case SpaceType.UTILITY:  return BOARD_TILE_COLORS.utility;
    case SpaceType.TAX:      return BOARD_TILE_COLORS.propertyRed;
    default:                 return BOARD_TILE_COLORS.propertyBlue;
  }
}
