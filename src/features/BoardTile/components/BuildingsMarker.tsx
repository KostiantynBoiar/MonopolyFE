...
// Sizing: vmin ≈ board size at typical aspect ratios.
// Must fit inside the color band (≈1.3vmin deep).
...
// Must match PROPERTY_HEADER_RATIO (1.25) in boardTile.constants.ts.
...
function getBandStyle(edge: TileEdge): React.CSSProperties {
  ...
  switch (edge) {
    case TileEdge.BOTTOM: return { ...base, top: 0, left: 0, right: 0, height: BAND_DEPTH, flexDirection: 'row' };
    case TileEdge.TOP:    return { ...base, bottom: 0, left: 0, right: 0, height: BAND_DEPTH, flexDirection: 'row' };
    case TileEdge.LEFT:   return { ...base, right: 0, top: 0, bottom: 0, width: BAND_DEPTH, flexDirection: 'column' };
    case TileEdge.RIGHT:  return { ...base, left: 0, top: 0, bottom: 0, width: BAND_DEPTH, flexDirection: 'column' };
    default:              return { ...base, top: 4, left: 4, flexDirection: 'row' };
  }
}

function Block({ w, h, bg }: { w: string; h: string; bg: string }) {
  return (
    <div
      style={{
        width: w, height: h, backgroundColor: bg, borderRadius: '2px',
        borderWidth: '1.5px', borderStyle: 'solid', borderColor: 'rgba(255,255,255,0.90)',
        boxShadow: '0 0 0 1px rgba(0,0,0,0.36), 0 1.5px 4px rgba(0,0,0,0.58)', flexShrink: 0,
      }}
    />
  );
}
